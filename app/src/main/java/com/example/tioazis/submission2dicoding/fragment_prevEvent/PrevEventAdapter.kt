...
class PrevEventAdapter(private val events:List<Event>, private val listener:(Event) -> Unit) :
        RecyclerView.Adapter<PrevEventAdapter.PrevEventHolder>(){

    override fun onBindViewHolder(holder: PrevEventHolder, position: Int) {
        holder.bindEvents(events [position],listener)
    }

    override fun getItemCount(): Int = events.size

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): PrevEventHolder {
        val inflater = LayoutInflater.from(parent.context)
        val view = inflater.inflate(R.layout.events,parent,false)

        return PrevEventHolder(view)

    }

    class PrevEventHolder(view:View):RecyclerView.ViewHolder(view){
